...
выполнил блокирующую задачу и спас релиз (не спрашивайте как), но качество этого кода оставляет желать лучшего.
...
1. Реализуйте юнит-тестирование класса `PageContainer`:
  ...
  * удостоверьтесь, что метод `AsyncSend` будет вызываться всегда, когда изменяются данные при загрузке +
  * удостоверьтесь, что метод `AsyncSend` будет вызываться всегда, при пропуске элемента +
...
1. Ниже есть некоторые мысли что можно улучшить, но хорошему студенту сначала рекомендуется выполнить работу самостоятельно, а затем свериться с подсказками.
